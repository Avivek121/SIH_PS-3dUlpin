...
async def get_system_health(db: AsyncSession = Depends(get_db)):
    """Live Database connection and API health status."""
    from sqlalchemy import text
    try:
        ver_res = await db.execute(text("SELECT version();"))
        db_ver = ver_res.scalar()
        db_status = "Connected"
    ...
    return {
        ...
        "database": {
            "status": db_status,
            "engine": db_ver.split()[0] + " " + db_ver.split()[1] if db_ver else "PostgreSQL",
        ...
        "backend_api": {
            ...
            "port": 8000,
            "swagger_docs": "http://localhost:8000/api/docs",
            "redoc_docs": "http://localhost:8000/api/redoc",
            "endpoints_total": 21
        }
    }
